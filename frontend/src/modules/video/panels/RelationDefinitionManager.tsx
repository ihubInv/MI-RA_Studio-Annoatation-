...
interface Props {
  schema: VideoRelationSchema
  onChange: (schema: VideoRelationSchema) => void
  onClose: () => void
}

export function RelationDefinitionManager({ schema, onChange, onClose }: Props) {
  const [selectedId, setSelectedId] = useState<string | null>(schema.relations[0]?.id ?? null)
  const selected = schema.relations.find((r) => r.id === selectedId) ?? null

  const update = (id: string, patch: Partial<RelationDefinition>) => {
    onChange({ ...schema, relations: schema.relations.map((r) => (r.id === id ? { ...r, ...patch } : r)) })
  }
          ...
          <ul className="w-44 border-r overflow-y-auto p-2 space-y-0.5">
            {schema.relations.map((r) => (
              <li key={r.id}>
                <button
                  type="button"
                  className={cn('w-full text-left px-2 py-1.5 rounded text-xs truncate flex gap-1.5 items-center', selectedId === r.id && 'bg-accent')}
            ...
            <li>
              <button
                ...
                onClick={() => {
                  const def = emptyRelationDefinition()
                  onChange({ ...schema, relations: [...schema.relations, def] })
          ...
          {selected && (
            <div className="flex-1 p-4 space-y-3 overflow-y-auto">
              ...
              <button type="button" className="text-xs text-destructive flex items-center gap-1" onClick={() => {
                onChange({ ...schema, relations: schema.relations.filter((r) => r.id !== selected.id) })
                setSelectedId(null)
              }}>
